...
function App() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!text) {
        setStatus({ kind: 'error', message: 'No extractable text was found in this PDF.' })
        return
      }
      setStatus({ kind: 'done', text })
    } catch (err) {
      const message = err instanceof PdfExtractionError ? err.message : 'Something went wrong extracting text from this PDF.'
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-3xl flex-col items-center gap-6 p-8">
      ...
      {status.kind === 'loading' && <p className="text-sm text-gray-500">Extracting text…</p>}
